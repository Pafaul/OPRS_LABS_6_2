...
int main()
{
    ...
    integrator->Run(c_grav_model); filename = "c_grav.txt"; writeRes(c_grav_model, filename);
    std::cout << " n_grav_model " << std::endl;
    integrator->Run(n_grav_model); filename = "n_grav.txt"; writeRes(n_grav_model, filename);
    std::cout << " p_grav_model " << std::endl;
    integrator->Run(p_grav_model); filename = "p_grav.txt"; writeRes(p_grav_model, filename);
